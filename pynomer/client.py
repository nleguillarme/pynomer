...
class NomerClient:
    ...
    def replace(self, query="", matcher="globi-taxon-cache", p=None):
        """
        ...
        """
        res = self.run_nomer(
            nomer_cmd="replace", query=query, matcher=matcher, p=self.get_properties(p)
        )
        return res["result"]

    def append(self, query="", matcher="globi-taxon-cache", p=None, o="tsv"):
        """
